...
class RequestGenerator extends Command
{
     ...
     * @var string
     */
    protected $signature = 'gcrud:makerequest
    ...
    /**
     * Generate Request in interactive mode
     * @throws \Illuminate\Contracts\Filesystem\FileNotFoundException
     */
    protected function interactive()
    {
        ...
        $this->comment("This command will guide you through creating your Request");
        $name = $this->ask('What is name of your Model?');
        $name = ucwords($name);
        $confirmOverwrite = $this->ask("If the file {$name}Request already exists, do you want it to be overwritten? [Y,n]") ?? 'y';
        ...
        $this->line("Name: $name");
        $this->line("Request: {$name}Request");
    ...
    {
        $this->comment("Generating {$name} Request");
