...
class TraitGenerator extends Command
{
     ...
     * @var string
     */
    protected $signature = 'gcrud:maketrait
     ...
     * @var string
     */
    protected $description = 'Create prepare trait with a multilayer structure';
    ...
    protected function interactive()
    {
        ...
        $name = ucwords($name);
        $confirmOverwrite = $this->ask("If the file {$name}Trait already exists, do you want it to be overwritten? [Y,n]") ?? 'y';
    ...
    protected function generate($name, $overwrite)
    {
        ...
        $this->comment("Generating {$name} Trait");
